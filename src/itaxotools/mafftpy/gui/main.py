...
from itaxotools import _mafft, common
from itaxotools.common import io, machine, resources, threading, widgets
...
class SeqEdit(QtWidgets.QTextEdit):
    ...
    def previewPath(self, path, max=-1, update=True):
        ...
        with path.open("r") as file:
            ...
            with io.redirect(_mafft, "stdout", os.devnull, "w"), io.redirect(
                _mafft, "stderr", os.devnull, "a"
            ):
...
class Main(widgets.ToolDialog):
    ...
    def __init__(self, parent=None, init=None):
        ...
        icon = QtGui.QIcon(
            common.resources.get("itaxotools.mafftpy.gui", "logos/mafft.ico")
        )
    ...
    def cog(self):
        ...
        self.state["idle/input.raw.unknown"] = QtStateMachine.QState(
            self.state["idle/input.raw"]
        )
        self.state["idle/input.raw.calculated"] = QtStateMachine.QState(
            self.state["idle/input.raw"]
        )
        self.state["idle/input.raw"].setInitialState(
            self.state["idle/input.raw.unknown"]
        )
        self.state["idle/input.last"] = QtStateMachine.QHistoryState(
            self.state["idle/input"]
        )
        self.state["idle/input.last"].setDefaultState(self.state["idle/input.none"])
        self.state["idle/input.last"].setHistoryType(
            QtStateMachine.QHistoryState.DeepHistory
        )
        ...
        self.state["idle/output.none"] = QtStateMachine.QState(
            self.state["idle/output"]
        )
        self.state["idle/output.complete"] = QtStateMachine.QState(
            self.state["idle/output"]
        )
        self.state["idle/output.failed"] = QtStateMachine.QState(
            self.state["idle/output"]
        )
        self.state["idle/output.updated"] = QtStateMachine.QState(
            self.state["idle/output"]
        )
        self.state["idle/output.last"] = QtStateMachine.QHistoryState(
            self.state["idle/output"]
        )
        ...
        state.assignProperty(
            self.pane["output"].labelFoot, "text", 'Click "Run" to align.'
        )
        ...
        def onEntry(event):
            self.pane["output"].title = "\u2023 Results [{}]".format(
        ...
        state.assignProperty(
            self.pane["output"].labelFoot, "text", "Aligning sequences, please wait..."
        )
    ...
    def skin(self):
        ...
        for group in scheme:
            for role in scheme[group]:
                palette.setColor(group, role, QtGui.QColor(color[scheme[group][role]]))
    ...
    def draw(self):
        ...
        self.subheader.icon.setPixmap(
            widgets.VectorPixmap(
                get_icon("arrow-right.svg"), colormap=self.colormap_icon_light
            )
        )
        ...
        self.radio["fftns1"].setToolTip(
            "Very fast; recommended for >2,000 sequences; progressive method"
        )
        self.radio["ginsi"] = QtWidgets.QRadioButton("G-INS-i")
        self.radio["ginsi"].setToolTip(
            "Very slow; recommended for <200 sequences with global homology"
        )
    ...
    def act(self):
        ...
        self.action["open"].setIcon(
            widgets.VectorIcon(get_icon("open.svg"), self.colormap)
        )
        ...
        self.action["save"].setIcon(
            widgets.VectorIcon(get_icon("save.svg"), self.colormap)
        )
        ...
        self.action["run"].setIcon(
            widgets.VectorIcon(get_icon("run.svg"), self.colormap)
        )
        ...
        self.action["stop"].setIcon(
            widgets.VectorIcon(get_icon("stop.svg"), self.colormap)
        )
        ...
        pixmap = widgets.VectorPixmap(
            get_icon("search.svg"), colormap=self.colormap_icon_light
        )
